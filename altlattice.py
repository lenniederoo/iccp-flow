...
def move(grid,blocks,bigblocks,e):
  ...
  for i in range(9):
    ...
    for j in range(len(bigblocks)):      
      grid[bigblocks[j,0],bigblocks[j,2]:bigblocks[j,3],i]=clonedgrid[bigblocks[j,0],bigblocks[j,2]:bigblocks[j,3],((i+3)%8) +1]
      grid[bigblocks[j,1],bigblocks[j,2]:bigblocks[j,3],i]=clonedgrid[bigblocks[j,1],bigblocks[j,2]:bigblocks[j,3],((i+3)%8) +1]
      grid[bigblocks[j,0]:bigblocks[j,1],bigblocks[j,2],i]=clonedgrid[bigblocks[j,0]:bigblocks[j,1],bigblocks[j,2],((i+3)%8) +1]
      grid[bigblocks[j,0]:bigblocks[j,1],bigblocks[j,3],i]=clonedgrid[bigblocks[j,0]:bigblocks[j,1],bigblocks[j,3],((i+3)%8) +1]  
  return grid  
...
def calc_velocity(grid,bigblocks):
    ...
    for j in range(len(bigblocks)):      
      velocity[bigblocks[j,0]-1:bigblocks[j,1]+1,bigblocks[j,2]-1:bigblocks[j,3]+1,:]=0    
    return velocity
    
def add_pressure_grad(grid,pressgradvel,blocks,bigblocks):
    velocity=calc_velocity(grid,bigblocks)
